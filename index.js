const { openFile } = require("node-file-dialog"); // Sorry windows users, could not get node-file-dialog to work on my machine.
...
async function selectFileFromSystem() {
    try {
        ...
        if (fileStats.isFile()) {
            ...
            console.log("The specified path does not point to a file.");
            return undefined;
        }
    } catch (error) {
        console.error("An error occurred while selecting the file:", error);
        return undefined;
    }
}
